...
//Added
void thread_sleep(int64_t ticks) { //Function to sleep the thread until ticks
  //if(thread_current != idle_thread) {
    //thread_block();
    struct thread *cur;
    
    enum intr_level old_level; // Save previous interrupt level
    old_level = intr_disable(); // Disable interrupt 
    
    cur = thread_current(); 
    ASSERT(cur != idle_thread); // Idle threads should not sleep
    update_next_tick_to_awake(cur-> wakeup_tick = ticks); //update the tick value at which the awake function should be executed
    
    list_push_back(&sleep_list, &cur->elem); //Schedule the current thread after inserting it into the sleep queue.
    
    thread_block(); //Block this thread and wait for it to be rescheduled
    
    intr_set_level(old_level); //Fix to accept interrupts again
  //}
}
...
thread_set_priority (int new_priority) 
{
  ...
  // Added code donations
  thread_refresh_priority();

  //Added to priority scheduler
  thread_test_preemption();
}
...
{ // Set the nice value of the current thread to a new value 
...
{ // Returns the current system load_avg * 100 
  enum intr_level old_level = intr_disable ();
...
{ // Returns the recent_cpu * 100 value of the current thread 
  enum intr_level old_level = intr_disable ();
...
void thread_refresh_priority (void) {
  ...
  if (!list_empty (&cur->donations)) {
    list_sort (&cur->donations, thread_cmp_thread_donate_priority, 0);
...
// Added code donations
bool thread_cmp_thread_donate_priority (const struct list_elem *l, 
